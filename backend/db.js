...
/**
 * Protokolliert, dass der Nutzer der sofortigen Ausführung zugestimmt
 * und damit sein Widerrufsrecht gemäß § 356 Abs. 5 BGB verloren hat.
 * Wird nur gesetzt, wenn die Checkbox im Checkout aktiv angehakt wurde.
 */
export async function recordWithdrawalWaiver(x, y) {
  await pool.query(
    `UPDATE pixels SET withdrawal_waiver_accepted_at = now(), updated_at = now()
     WHERE x = $1 AND y = $2`,
    [x, y]
  );
}
