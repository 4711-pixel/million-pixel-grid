...
app.post('/api/pixels/:x/:y/checkout', async (req, res) => {
  ...
  const { color, withdrawalWaiverAccepted } = req.body;
  ...
  // Ohne diese explizite Zustimmung darf der Checkout nicht starten – sie ist
  // Voraussetzung dafür, dass das gesetzliche Widerrufsrecht gemäß § 356
  // Abs. 5 BGB überhaupt wirksam erlischt (siehe AGB § 6).
  if (withdrawalWaiverAccepted !== true) {
    return res.status(400).json({ error: 'Der Zustimmung zur sofortigen Ausführung wurde nicht zugestimmt.' });
  }
  ...
  try {
    await recordWithdrawalWaiver(x, y);
